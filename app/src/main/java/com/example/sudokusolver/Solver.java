...
public class Solver {
    ...
    protected boolean check(int row, int col){
    ...
    public boolean solve(Board display){
        for(int r=0;r<9;r++){
            for(int c=0;c<9;c++){
                if(!check(r,c)){
                    return false;
        ...
        for (int i=1; i<10; i++){
            ...
            display.invalidate(); //prikaz svakog koraka
            if (check(row,col)){
                if(solve(display)){
                    return true;
                }
    ...
    public void setNumberPos(int num){
        if(this.selected_column != -1 && this.selected_row != -1){
